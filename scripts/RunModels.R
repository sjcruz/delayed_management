...
# I might need to come back and also save all biomass trajectories for each viable r-k pair
...
MPA.mat <- as.matrix(read.csv(here::here("inputs", "MPA.matrix.csv")))
...
BAU_out <- MPA_size_fun (data=BAU_inputs, years=115, MPA.mat = MPA.mat, start.year=0)
...
system.time({
  ...
  scenarios <- foreach(i = seq(2015, 2030, 5), .combine = rbind) %do%
    MPA_size_fun (data=inputs, years=100, MPA.mat=MPA.mat, start.year=i)
})


plot<- scenarios%>%
  filter(Reserve_size==0.05)%>%filter(Implementation_year==2030)
...
BAU <- read.csv(here::here("inputs", "BAU.csv"))%>%
  plyr::rename(c("Catch"="BAU_C", "Biomass"="BAU_B", "PV"="BAU_PV"))%>%
  select(Name, Adjusted, Year, BAU_C, BAU_B, BAU_PV)

scenarios <- read.csv(here::here("inputs", "Scenarios.csv"))

payoff_analysis <- payoff(BAU=BAU, scenarios=scenarios, type="Fishery")

write.csv(payoff_analysis, file.path("inputs", "payoff_fishery.csv"))
