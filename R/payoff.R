...
payoff <- function(BAU, scenarios, type){
  ...
  if (type=="All"){
   scen <- All %>% group_by(Adjusted, Implementation_year, Reserve_size, Year)%>%
     summarise_at(.vars= vars(Catch, Biomass, PV, BAU_C, BAU_B, BAU_PV), .funs =  sum)%>%
   ...
   cross_point <- scen %>%  group_by(Adjusted, Implementation_year, Reserve_size)%>% 
     subset(ifelse(Adjusted=="IUU_20_legal"|Adjusted=="IUU_40_legal"|Adjusted=="IUU_60_legal",     
                   Catch >= BAU_C, Year >= Implementation_year & Catch >= BAU_C))%>% 
              filter(Year==min(Year))%>%
     mutate(Transition_period= Year, Loss = BAU_C_cum - C_cum)%>%
  ...
  B_50 <- scen %>% group_by(Adjusted, Implementation_year, Reserve_size) %>%
    ...
    output <- All %>%
      ...
      scen <- All %>% group_by(Name, Adjusted, Implementation_year, Reserve_size)%>%
        arrange(Year, .by_group = TRUE)%>%
        mutate (C_cum = cumsum(Catch), BAU_C_cum = cumsum(BAU_C))
